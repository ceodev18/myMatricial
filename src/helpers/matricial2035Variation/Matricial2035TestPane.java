...
public class Matricial2035TestPane extends JPanel {
	private static final long serialVersionUID = 1L;
	private Matricial2035LandMap matricial2035LandMap;
	public String Matricial2035String;
	private int growtXY = 1;
	private int large = 35;
	private int width = 35;
	private int stringedType;
	private List<Matricial2035LandRoute> landRoutes;
	private List<Integer> landmapNodes;

	public Matricial2035TestPane(Matricial2035LandMap matricial2035LandMap, List<Matricial2035LandRoute> landRoutes, int large, int width) {
		this.stringedType = 0;
		this.matricial2035LandMap = matricial2035LandMap;
		this.landRoutes = landRoutes;
		this.large = large;
		this.width = width;
	}

	public Matricial2035TestPane(int stringedType, List<Integer> landmapNodes, String stringify,
			List<Matricial2035LandRoute> landRoutes, int large, int width) {
		this.large = large;
		this.width = width;
		this.stringedType = stringedType;
		this.landRoutes = landRoutes;
		this.Matricial2035String = stringify;
		this.landmapNodes = landmapNodes;
	}

	public Matricial2035TestPane(int stringedType, String stringify, List<Matricial2035LandRoute> landRoutes, int large, int width) {
		this.large = large;
		this.width = width;
		this.stringedType = stringedType;
		this.landRoutes = landRoutes;
	...
	private void linearGrammarSimulation(Graphics g) {
		String[] mapLines = Matricial2035String.split("\\,");
		for (int i = 0; i < landmapNodes.size(); i++) {
			int[] initialXY = Matricial2035MapHelper.breakKey(landmapNodes.get(i));
			int[] finalXY = Matricial2035MapHelper.breakKey(landmapNodes.get((i + 1) % landmapNodes.size()));
		...
		for (int i = 0; i < landRoutes.size(); i++) {
			int xy[] = Matricial2035MapHelper.breakKey(landRoutes.get(i).getInitialPointId());
	...
	private void normalSimulation(Graphics g) {
		...
		for (int y = 0; y < matricial2035LandMap.getPointsy(); y++) {
			for (int x = 0; x < matricial2035LandMap.getPointsx(); x++) {
				String type = matricial2035LandMap.findPoint(Matricial2035MapHelper.formKey(x, y)).getType();
				switch (type) {
				case "a":
				case "b":
				case "c":
