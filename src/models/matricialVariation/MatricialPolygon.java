...
public class MatricialPolygon {
	...
	public List<Integer> centroidShrinking(int size) {
		...
		for (int i = 0; i < points.size(); i++) {
			int[] xy = MatricialMapHelper.breakKey(points.get(i));
			...
			shrinkedList.add(i, MatricialMapHelper.formKey(xy[0], xy[1]));
		}
		return shrinkedList;
	}
	...
	public List<Integer> vectorShrinking(int size) {
		...
		MatricialLandMap auxLandMap = new MatricialLandMap(0, 0);
		//verify if "size" distance to centroid is more that the permissible
		double minDistance=999999;
		for(int r=0;r < points.size(); r++){
			int auxPoint = auxLandMap.findProyectionPointIntoParalelStraights(points.get(r),points.get((r+1)%points.size()),MatricialMapHelper.formKey(centroid[0],centroid[1]),true); 
			double auxDistance = auxLandMap.distanceOfPointToPoint(auxPoint, MatricialMapHelper.formKey(centroid[0],centroid[1]));
		...
		//////////////////
		for (int i = 0; i < points.size(); i++) {
			int[] xyInitial = MatricialMapHelper.breakKey(points.get(i));
			int[] xyFinal = MatricialMapHelper.breakKey(points.get((i + 1) % points.size()));
			...
			int auxPoint2 =auxLandMap.findProyectionPointIntoParalelStraights(points.get(i),points.get((i + 1) % points.size()),MatricialMapHelper.formKey(centroid[0],centroid[1]),false);
			int auxPoint3 = auxLandMap.findPointOnStreightToDistance(auxPoint2, MatricialMapHelper.formKey(centroid[0],centroid[1]), size);
			int[] xyRef = MatricialMapHelper.breakKey(auxPoint3);
			*/
		...
		for (int i = 0; i < offsets.size(); i++) {
			...
			if (gradients.get(i).isInfinite()) {
				infinite = 1;
				int[] xAux = MatricialMapHelper.breakKey(points.get(i));
			...
			if (gradients.get(previous).isInfinite()) {
				int[] xAux = MatricialMapHelper.breakKey(points.get(previous));
				infinite = 2;
			...
			if (gradients.get(i) == 0.0) {
				zero = 1;
				int[] xyAux = MatricialMapHelper.breakKey(points.get(i));
			...
			if (gradients.get(previous) == 0.0) {
				zero = 2;
				int[] xyAux = MatricialMapHelper.breakKey(points.get(previous));
			...
			if (infinite > 0 && zero > 0) {
				shrinkedList.add(MatricialMapHelper.formKey(xy[0], xy[1]));
				continue;
			}
	...
	private boolean crossCheck(List<Integer> shrinkedList) {
		for (int i = 0; i < shrinkedList.size(); i++) {
			int[] in1XY = MatricialMapHelper.breakKey(shrinkedList.get(i));
			int[] fi1XY = MatricialMapHelper.breakKey(shrinkedList.get((i + 1) % shrinkedList.size()));
			int j = 0;

			while ((i + 3 + j) % shrinkedList.size() != i) {
				int[] in2XY = MatricialMapHelper.breakKey(shrinkedList.get((i + 2 + j) % shrinkedList.size()));
				int[] fi2XY = MatricialMapHelper.breakKey(shrinkedList.get((i + 3 + j) % shrinkedList.size()));
	...
	private boolean isInsidePolygon(Integer vertexId) {
		boolean c = false;
		int[] xy = MatricialMapHelper.breakKey(vertexId);
		for (int i = 0, j = getPoints().size() - 1; i < getPoints().size(); j = i++) {
			if (((MatricialMapHelper.breakKey(getPoints().get(i))[1] > xy[1]) != (MatricialMapHelper
					.breakKey(getPoints().get(j))[1] > xy[1]))
					&& (xy[0] < (MatricialMapHelper.breakKey(getPoints().get(j))[0]
							- MatricialMapHelper.breakKey(getPoints().get(i))[0])
							* (xy[1] - MatricialMapHelper.breakKey(getPoints().get(i))[1])
							/ (MatricialMapHelper.breakKey(getPoints().get(j))[1]
									- MatricialMapHelper.breakKey(getPoints().get(i))[1])
							+ MatricialMapHelper.breakKey(getPoints().get(i))[0]))
				c = !c;
		}
		return c;
	...
	private double minimunDistanceBetweenVertex(List<Integer> area) {
		...
		for (int i = 0; i < area.size(); i++) {
			int[] xyInitial = MatricialMapHelper.breakKey(area.get(i));
			int[] xyFinal = MatricialMapHelper.breakKey(area.get((i + 1) % area.size()));
	...
	public void rehashPolygon(int type) {
		...
		if (type == MatricialConfiguration.TYPE_SPECIAL) {
	...
	public double areaShrinking(int size){
		...
		MatricialLandMap auxLandMap = new MatricialLandMap(0, 0);
		List<MatricialLandPoint> auxPolygon = new ArrayList<>();		
		for(int i = 0; i < aux.size();i++){
			int[] valxy = MatricialMapHelper.breakKey(aux.get(i));
			MatricialLandPoint landPoint = new MatricialLandPoint(valxy[0],valxy[1]);
			auxPolygon.add(landPoint);
		}
		int[] valxy = MatricialMapHelper.breakKey(aux.get(0));
		MatricialLandPoint landPoint = new MatricialLandPoint(valxy[0],valxy[1]);
		auxPolygon.add(landPoint);
