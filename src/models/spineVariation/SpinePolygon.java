...
public class SpinePolygon {
	...
	public List<Integer> centroidShrinking(int size) {
		...
		for (int i = 0; i < points.size(); i++) {
			int[] xy = SpineMapHelper.breakKey(points.get(i));
			...
			shrinkedList.add(i, SpineMapHelper.formKey(xy[0], xy[1]));
		}
		return shrinkedList;
	}
	...
	public List<Integer> vectorShrinking(int size) {
		...
		for (int i = 0; i < points.size(); i++) {
			int[] xyInitial = SpineMapHelper.breakKey(points.get(i));
			int[] xyFinal = SpineMapHelper.breakKey(points.get((i + 1) % points.size()));
		...
		for (int i = 0; i < offsets.size(); i++) {
			...
			if (infinite > 0 && zero > 0) {
				shrinkedList.add(SpineMapHelper.formKey(xy[0], xy[1]));
				continue;
			}
	...
	public boolean isInsidePolygon(Integer vertexId) {
		...
		for (int i = 0, j = getPoints().size() - 1; i < getPoints().size(); j = i++) {
			if (((SpineMapHelper.breakKey(getPoints().get(i))[1] > xy[1]) != (SpineMapHelper
					.breakKey(getPoints().get(j))[1] > xy[1]))
					&& (xy[0] < (SpineMapHelper.breakKey(getPoints().get(j))[0]
							- SpineMapHelper.breakKey(getPoints().get(i))[0])
							* (xy[1] - SpineMapHelper.breakKey(getPoints().get(i))[1])
							/ (SpineMapHelper.breakKey(getPoints().get(j))[1]
									- SpineMapHelper.breakKey(getPoints().get(i))[1])
							+ SpineMapHelper.breakKey(getPoints().get(i))[0]))
				c = !c;
		}
		return c;
	...
	private double minimunDistanceBetweenVertex(List<Integer> area) {
		...
		for (int i = 0; i < area.size(); i++) {
			int[] xyInitial = SpineMapHelper.breakKey(area.get(i));
			int[] xyFinal = SpineMapHelper.breakKey(area.get((i + 1) % area.size()));
	...
	public void rehashPolygon(int type) {
		...
		if (type == SpineConfiguration.TYPE_SPECIAL) {
