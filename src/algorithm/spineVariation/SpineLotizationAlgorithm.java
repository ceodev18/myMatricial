...
public class SpineLotizationAlgorithm {
	...
	private static void organicZonification(){
		...
		for (int j = spineLandMap.getPointsy() - 1; j >= 0; j--) {
			for (int i = 0; i < spineLandMap.getPointsx(); i++) {				
				...
				//if(i== xx && j==yy)spineLandMap.getLandPoint(SpineSpineMapHelper.formKey(i,j)).setType(SpineConfiguration.NODE_MARK);
			}
		}
	}
	private static void changeValue(int xx,int yy){
		...
		for(int i=0;i<large;i++){
			//caso 1 
			if(spineLandMap.findPoint(SpineMapHelper.formKey(i,yy)).getType().equals(SpineConfiguration.COLLECTOR_MARK)){
				i+=16;
				indice=i;
		...
		//spineLandMap.getLandPoint(SpineSpineMapHelper.formKey(indice,yy)).setType("K");
		int tp1,tp2,tp3,tp4;
		tp3=tp4=0;
		String etq1,etq2;
		...
		for(int i=0;i<large;i++){
			tp1=0;
			 ...
			 int j=0;
			 while(true){
				 if(spineLandMap.findPoint(SpineMapHelper.formKey(i,yy-j)).getType().equals(SpineConfiguration.POLYGON_BORDER) ||
						 spineLandMap.findPoint(SpineMapHelper.formKey(i,yy-j)).getType().equals(SpineConfiguration.OUTSIDE_POLYGON_MARK))break;
				 if(spineLandMap.findPoint(SpineMapHelper.formKey(i,yy-j)).getType().equals(SpineConfiguration.EMPTY_MARK)){
					 ...
					 if(div>=1 && j>=Integer.parseInt(SpineConfiguration.BASE_CLUSTER_SIZE+"") && (j%Integer.parseInt(SpineConfiguration.BASE_CLUSTER_SIZE+""))==0 ){
						 ...
						 //if(spineLandMap.findPoint(SpineMapHelper.formKey(i,yy-j)).getType().equals(SpineConfiguration.POLYGON_BORDER) ||
								// spineLandMap.findPoint(SpineMapHelper.formKey(i,yy-j)).getType().equals(SpineConfiguration.OUTSIDE_POLYGON_MARK))break;
						 for(int k=0;k<10;k++){
							 if(spineLandMap.findPoint(SpineMapHelper.formKey(i,yy-j-k)).getType().equals(SpineConfiguration.POLYGON_BORDER)||
									 spineLandMap.findPoint(SpineMapHelper.formKey(i,yy-j-k)).getType().equals(SpineConfiguration.OUTSIDE_POLYGON_MARK))break;
						 ...
						 if(spineLandMap.findPoint(SpineMapHelper.formKey(i,yy-j)).getType().equals(SpineConfiguration.POLYGON_BORDER) ||
								 spineLandMap.findPoint(SpineMapHelper.formKey(i,yy-j)).getType().equals(SpineConfiguration.OUTSIDE_POLYGON_MARK))break;
						 for(int l=0;l<27;l++){
							 if(spineLandMap.findPoint(SpineMapHelper.formKey(i,yy-j-l)).getType().equals(SpineConfiguration.POLYGON_BORDER)||
									 spineLandMap.findPoint(SpineMapHelper.formKey(i,yy-j-l)).getType().equals(SpineConfiguration.OUTSIDE_POLYGON_MARK))break;
							 //if(spineLandMap.findPoint(SpineMapHelper.formKey(i,yy-j-l)).getType().equals(SpineConfiguration.POLYGON_BORDER) ||
								//	 spineLandMap.findPoint(SpineMapHelper.formKey(i,yy-j-l)).getType().equals(SpineConfiguration.OUTSIDE_POLYGON_MARK))break;
						...
						// spineLandMap.getLandPoint(SpineSpineMapHelper.formKey(i,yy-j)).setType("p");
						 //continue;
					 }
					 if(tp3<27){
						...
						// spineLandMap.getLandPoint(SpineSpineMapHelper.formKey(i,yy-j)).setType("p");
					 	...
					 	if(tp3<27){
							...
							spineLandMap.getLandPoint(SpineMapHelper.formKey(i,yy+j)).setType(SpineConfiguration.PARK_MARK);	
					 		//spineLandMap.getLandPoint(SpineSpineMapHelper.formKey(i,yy+j)).setType(valuePrint);	
							tp3++;
